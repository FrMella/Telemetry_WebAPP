<?php
/*
* Admin control panel para visualizar la aplicacion de telemetria de los datos
*/
...
function admin_controller()
{
    ...
    require_once "Ext_Modules/admin/admin_model.php";
    ...
    if ($route->action == 'db' && ($session['admin'] || $settings['updatelogin']===true)) {
        ...
        require_once "Libraries/dbschemasetup.php";
        $updates = array(array(
        ...
        return view("Ext_Modules/admin/Views/mysql_update_view.php", array('applychanges'=>$applychanges, 'updates'=>$updates, 'error'=>$error));
    }
    ...
    if ($route->action == 'info') {
        $route->format = 'html';
        return view("Ext_Modules/admin/Views/admin_main_view.php", $admin->full_system_information());
    }
    
    if ($route->action == 'update') {
        $route->format = 'html';
        return view("Ext_Modules/admin/Views/update_view.php", array(
    ...
    if ($route->action == 'components') {
        $route->format = 'html';
        return view("Ext_Modules/admin/Views/components_view.php", array(
    ...
    if ($route->action == 'serial') {
        $route->format = 'html';
        return view("Ext_Modules/admin/Views/serialmonitor_view.php", array(
            'serial_ports'=>$admin->listSerialPorts()
        ));
    }
    ...
    if ($route->action == 'log') {
        ...
        return view("Ext_Modules/admin/Views/appTelemetry_logView.php", array(
            'log_enabled'=>$settings['log']['enabled'],
            'app_logfile'=>$admin->app_logfile(),
    ...
    if ($route->action == 'users') {
        $route->format = 'html';
        return view("Ext_Modules/admin/Views/userlist_view.php", array());
    }
    ...
    if ($route->action == 'downloadlog') {
        if ($settings['log']['enabled']) {
            header("Content-Type: application/octet-stream");
            header("Content-Transfer-Encoding: Binary");
            header("Content-disposition: attachment; filename=\"" . basename($admin->app_logfile()) . "\"");
            header("Pragma: no-cache");
            header("Expires: 0");
            flush();
            if (file_exists($admin->app_logfile())) {
                readfile($admin->app_logfile());
            } else {
                echo($admin->app_logfile() . " does not exist!");
            }
            exit;
        }
        return false;
    ...
    if ($route->action == 'getlog') {
        ...
        else if (!file_exists($admin->app_logfile())) { 
            $route->format = "json";
            return array('success'=>false, 'message'=>$admin->app_logfile() . " does not exist");
        }
        ...
        $fsize = round(filesize($admin->app_logfile())/1024/1024,2);
        $lines = read_file($admin->app_logfile(), 25);
