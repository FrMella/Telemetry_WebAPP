...
function feed_controller()
{
    ...
    if ($route->format == 'html')
    {
        ...
        if (($route->action == "view" || $route->action == "list") && $session['write']) {
            return view("Ext_Modules/feed/Views/feedlist_view.php");
        }
        else if ($route->action == "api" && $session['write']) {
            require "Ext_Modules/feed/feed_api_obj.php";
            return view("Libraries/api_tool_view.php",array("title"=>_("Feed API"), "api"=>feed_api_obj(), "selected_api"=>9));
        }
