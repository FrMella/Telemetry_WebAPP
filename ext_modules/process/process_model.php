...
class Process
{
            ...
            if ($steps > $total*2) {
                ...
                switch ($options['sourcetype']) {
                    ...
                    case ProcessOriginType::TASK:
                        if (file_exists("Ext_Modules/task/task_model.php")) {
                            global $session, $redis;
                            require_once "Ext_Modules/task/task_model.php";
    ...
    private function load_modules() {
        ...
        for ($i=2; $i<count($dir); $i++) {
            $module = $dir[$i];
            if (filetype("Ext_Modules/$module")=='dir' || filetype("Ext_Modules/$module")=='link') {
    ...
    private function get_module_class($module_name){
        /*
            ...
            Looks in the folder Ext_Modules/modulename/ for a file modulename_processlist.php 
            (module_name all lowercase but class Modulename_ProcessList in php file that is with upper case first letter)
        */
        $module_file = "Ext_Modules/".$module_name."/".$module_name."_processlist.php";
        $module_class=null;
