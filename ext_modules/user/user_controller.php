...
function user_controller()
{
    ...
    if ($route->format == 'html')
    {
        if ($route->action == 'login' && !$session['read']) {
            ...
            $result = view("Ext_Modules/user/login_block.php", array(
        ...
        if ($route->action == 'view' && $session['write']) $result = view("Ext_Modules/user/profile/profile.php", array());
        ...
        if ($route->action == 'verify' && $settings["interface"]["email_verification"] && !$session['read'] && isset($_GET['key'])) { 
            ...
            $result = view("Ext_Modules/user/login_block.php", array('allowusersregister'=>$allowusersregister,'verify'=>$verify));
        }
    }
    ...
    if ($route->format == 'json')
    {
        ...
        if ($route->action == "deleteall" && $session['write']) {
            ...
            require "Ext_Modules/user/deleteuser.php";
