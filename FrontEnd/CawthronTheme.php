...
<head>
    ...
    <link href="<?php echo $path; ?>Libraries/bootstrap/css/bootstrap-responsive.min.css" rel="stylesheet">
        ...
        <div class="menu-tr"><ul>
                ...
                <?php if ($session["write"]) { ?>
                    <li class="<?php echo $session["gravatar"]?'':'no-'; ?>gravitar dropdown"><a id="user-dropdown" href="#" title="<?php echo $session["username"]." ".($session['admin']?'(Admin)':'')?>" class="grav-container img-circle d-flex dropdown-toggle" data-toggle="dropdown">
                            ...
                            <?php } else { ?>
                               <img src="<?php echo md5($session["gravatar"]); ?>?s=52&d=mp&r=g" class="grav img-circle"> /* cambia la foto de perfil */
                            <?php } ?>
                        </a>

                        <ul class="dropdown-menu pull-right" style="font-size:1rem">
                            <li><a href="<?php echo $path; ?>user/view" title="<?php echo _("Profile"); ?>" style="line-height:30px"><svg class="icon"><use xlink:href="#icon-user"></use></svg> <?php echo _("Profile"); ?></a></li>
